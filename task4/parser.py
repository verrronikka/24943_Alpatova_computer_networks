...
async def create_dataset_and_db(styles, tags, page, data_dir, max_pins, scrolls):
    data = []
    ...
    df.to_sql("pins", con=engine, if_exists="append", index=False)
...
async def scrape_pinterest(state_path, styles, tags, max_pins, scrolls):
    async with async_playwright() as p:
        try:
            ...
            await create_dataset_and_db(styles, tags, page, data_dir, max_pins, scrolls)
...
def run_parser_sync(state_path, styles, tags, max_pins, scrolls):
    return asyncio.run(scrape_pinterest(state_path, styles, tags, max_pins, scrolls))
